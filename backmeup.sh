...
description="A tool to automate bugbounty process as: Tool will execute multiple tools to collect URLs from internet archives then use some useful patterns/RegEx to look for Sensitive Data Leakage in the form of multiple juicy extensions."
...
found(){
    if [[ -f ${target} ]] && [[ -e ${target} ]] && [[ -r ${target} ]];then
        cat ${BASE_DIR}/.hold_regex.txt | while IFS= read -r rr || [[ -n ${rr} ]]; do
            if [ -f ${BASE_DIR}/output/{}_$(date -I)/{}_*.txt ];then

                cat ${target} | parallel --load 100% --timeout 100% --retries 1 -j+0 -k --lb --compress --joblog ${BASE_DIR}/backmeup_logs --silent "egrep -ai --no-filename \"${rr}\" ${BASE_DIR}/output/{}_$(date -I)/{}_*.txt | anew -q ${BASE_DIR}/output/{}_$(date -I)/{}_possible_leaked_data.txt"
            else
        ...
        cat ${BASE_DIR}/.hold_regex.txt | while IFS= read -r rr || [[ -n ${rr} ]]; do
            if [ -f ${BASE_DIR}/output/{}_$(date -I)/{}_*.txt ];then
                echo ${target} | parallel --load 100% --timeout 100% --retries 1 -j+0 -k --lb --compress --joblog ${BASE_DIR}/ram_log --silent "egrep -ai --no-filename \"${rr}\" ${BASE_DIR}/output/{}_$(date -I)/{}_*.txt | anew -q ${BASE_DIR}/output/{}_$(date -I)/{}_possible_leaked_data.txt"
            else
                printf "[${red}${cross}${end}] ${yellow}No data ! Bad luck on the target ${red}\"${target}\"${end}\n"
                exit 0
            fi
